...
Solution: Créer une nouvelle colonne avec la fonction *cut* et utiliser transition_manual(). Ainsi, toutes les éruptions sont représentés. Nous avons utilisé la distribution des années pour créer les catégories. Chaque image présente environ 0.0025% des éruptions du jeu de données. Plus le temps avance, plus nous avons des données et on peut voir la longueur des catégories rapetisser dans le temps.
...
# Dans l'application, nous avons utilisé fps=1 et nous avons exporté le gif en vidéo. 
# Cela donne plus de contrôle à l'utilisateur pour aller voir des années ciblées. 
# L'animation plus lente est aussi plus facile à interpréter. 
```
