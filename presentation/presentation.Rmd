...
ggplot(
  ...
  theme_classic() +
  labs(
    title="Proportion de volcans par région selon \nl'indice de force de ses éruptions",
...
boxplot(
  ...
  lwd = 1.5,
  main = "Année où un événement à débuté \npar indice de force de ses éruptions",
  xlab = "Indice de force",
  ylab = "Année"
)
```
...
ggplot(
  ...
  theme_classic() +
  labs(
    title="Proportion de volcans par type d'événement \nselon l'indice de force de ses éruptions",
...
ggplot(
  ...
  theme_classic() +
  labs(
    title="Proportion de volcans par type de volcan \nselon l'indice de force de ses éruptions",
...
---
# Résultat:
```{r imageplot2004, echo=FALSE, out.width="70%", fig.align = 'center'}
include_graphics("img/plot_2004.png")
```

---
# Carte animée - Exploration
Voici maintenant le code pour générer un graphique statique qui regroupe toutes les éruptions de 1812 séparées par indice de force. Cette approche a été testée en animation, mais elle avait pour désavantage d'avoir un temps d'exportation très long et c'était aussi difficile d'interpréter 7 animations à la fois!
...
L'animation finale est basée sur les paramètres du graphique statique de 2004, mais ici l'année a été utilisée comme variable d'animation. Nous avons eu quelques difficultées pour trouver comment bien représenter les données dans l'animation:
- transition_time() utilise fait des saut équidistants et fait abstraction des années manquantes dans le jeu de données
- transition_manual() ne cible que les données présentes, mais si on ne veut pas une image par année (1540 images dans notre cas), alors le bond dans le temps est calculé automatiquement en fonction de *nframes* et de *fps* et certaines années sont simplement exclues de l'animation.

Solution: Créer une nouvelle colonne avec la fonction *bins* et utiliser transition_manual(). Ainsi, toutes les éruptions sont représentés. Nous avons utilisé la distribution des années pour créer les catégories. Chaque image présente environ 0.00333% des éruptions du jeu de données. Plus le temps avance, plus nous avons des données et on peut donc voir la longueur des catégories rapetisser dans le temps.

---
# Carte animée finale suite

Voici le code qui a permis de généré le gif à la page suivante. 

L'option transition_time() a été testée, mais cette option fait des bonds équidistants dans le temps en fonction du nombre d'images et du fps. Or, nous avons beaucoup d'années absentes du jeu de données. Nous avons donc utilisé l'option transition_manual() pour animer seulement sur les années présentes dans le jeu de données.
...
gifannees<-animate(mp7,fps=2,width=900, height=600, nframes=308)
# Dans l'application, nous avons utilisé *fps=1* et nous avons exporté le gif en vidéo. Cela donne plus de contrôle à l'utilisateur pour aller voir des années ciblées. L'animation plus lente est aussi plus facile à interpréter. 
anim_save(filename="ToutesAnnées.gif", gifannees)
```

---
# Résultat:
```{r gif, echo=FALSE, out.width="75%", fig.align = 'center'}
include_graphics("img/ToutesAnnées.gif")
#(ne loop pas... mettre en video comme sur l'app?)  
```
